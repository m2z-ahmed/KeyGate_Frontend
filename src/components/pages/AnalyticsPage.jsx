    ...
    <section className='page active analytics-page'>
      <div className='page-inner'>
        ...
        <div className='stats mobile-quick-stats'>
          ...
          <div className='stat'><div className='stat-val'>{isLoading ? '—' : `${analytics.avgLatency || 0}ms`}</div><div className='stat-label'>Avg latency</div></div>
          <div className='stat'><div className='stat-val'>{isLoading ? '—' : `$${totalCost.toFixed(4)}`}</div><div className='stat-label'>Est. cost</div></div>
        </div>
        ...
        <div className='analytics-grid'>
          ...
          <div className='card'>
            ...
            {!analytics.topModels?.length ? <div className='empty'><div className='empty-text'>No model usage yet</div></div>
              : analytics.topModels.map((m) => <div className='analytics-row' key={m.model}><span>{m.model}</span><strong>{fmtNum(m.count)}</strong></div>)}
          </div>
          <div className='card'>
            ...
            {!Object.keys(providers).length ? <div className='empty'><div className='empty-text'>No provider traffic yet</div></div>
              : Object.entries(providers).sort((a, b) => b[1] - a[1]).map(([provider, count]) => <div className='analytics-row' key={provider}><span>{provider}</span><strong>{fmtNum(count)}</strong></div>)}
          </div>
        </div>
        ...
        <div className='card'>
          ...
          {!logs.length ? <div className='empty'><div className='empty-text'>No requests logged yet</div></div>
            : logs.slice(0, 10).map((l) => <div className='analytics-row' key={l.id}><span>{l.subkey_name || 'Unknown subkey'} · {l.model || 'unknown'} · {l.status}</span><strong>{fmtTime(l.created_at)}</strong></div>)}
        </div>
      </div>
    </section>
  );
