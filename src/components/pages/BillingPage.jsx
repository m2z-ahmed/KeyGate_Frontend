...
const readStoredSubscriptionDetails = () => {
  try { return JSON.parse(localStorage.getItem('lethem_subscription_details') || 'null'); } catch { return null; }
};
...
export default function BillingPage({ ctx, onBack }) {
  ...
  const startCheckout = async (plan) => {
    ...
    try {
      ...
      const rz = new window.Razorpay({
        key: sub.keyId || billing.keyId, name: 'Lethem',
        description: `${plan.name} account subscription`,
        subscription_id: sub.subscriptionId, notes: { plan: plan.id }, theme: { color: '#7c6bff' },
        handler: async (response) => {
          ...
          setBilling(fresh); setCtxBilling?.(fresh); writeStoredSubscriptionDetails(fresh);
          setStoredDetails(readStoredSubscriptionDetails()); notify(`You're now on ${plan.name}`);
        },
  ...
  if (!billing && !storedDetails) return <div className='page active'><div className='page-inner'><div className='card'>Loading billing plans…</div></div></div>;

  return (
    <div className='billing-page page active account-billing-page'>
      <div className='page-inner'>
        {onBack && <button className='btn btn-ghost btn-sm billing-back' onClick={onBack}>← Back</button>}
        <div className='page-head billing-hero'>
          <div>
            <span className='eyebrow'>Account subscription</span>
            <h1>Plans &amp; Billing</h1>
            <p>Subscriptions are account-level. Upgrade once and your plan limits apply across your Lethem workspace.</p>
          </div>
        ...
        <div className='billing-current card billing-current-premium'>
          ...
          <div><div className='muted'>Currency</div><strong>{billing?.currency || storedDetails?.currency || 'INR'}</strong></div>
        </div>
        <div className='pricing-grid pricing-grid-premium'>
          {plans.map((plan) => (
            <div key={plan.id} className={`card pricing-card premium-pricing-card${plan.popular ? ' popular' : ''}`}>
              ...
              <div className='pricing-card-top'><h2>{plan.name}</h2><p>{plan.description}</p></div>
              <div className='price-line'><span>${plan.monthlyUsd}</span><small>/month</small></div>
              <div className='inr-line'>{plan.monthlyInr ? `₹${plan.monthlyInr.toLocaleString('en-IN')} via Razorpay` : 'No payment required'}</div>
