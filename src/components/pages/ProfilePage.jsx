    ...
    <section className='page active profile-page'>
      <div className='page-inner'>
        <div className='page-header'>
          ...
          <p className='page-sub'>Edit your display name. Your sign-in email is managed by Auth0.</p>
        </div>
        <div className='profile-grid'>
          <form className='card profile-editor-card' onSubmit={saveProfile}>
            <div className='card-header'>
              <div><div className='card-title'>Personal details</div><div className='card-sub'>Profile data is cached for fast loads and refreshed after saving.</div></div>
              <span className='badge active'>{loading ? 'Loading' : 'Editable'}</span>
            </div>
            <div className='form-row single'><div className='field'><label>Display name</label><input value={form.name} onChange={(e) => setForm((v) => ({ ...v, name: e.target.value }))} placeholder='Your name' disabled={loading || saving} /></div></div>
            <div className='form-row single'><div className='field'><label>Email</label><input type='email' value={form.email} readOnly disabled aria-readonly='true' title='Email is managed by your Auth0 sign-in account.' /><small className='field-help'>Email editing is disabled — your Lethem account is aligned with Auth0.</small></div></div>
            <div className='modal-footer'><button type='button' className='btn btn-ghost' disabled={!changed || saving || loading} onClick={() => setForm(original)}>Reset</button><button type='submit' className='btn btn-primary' disabled={!changed || saving || loading}>{saving ? 'Saving…' : 'Save changes'}</button></div>
          </form>
