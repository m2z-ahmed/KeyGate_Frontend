...
export default function MasterKeysPage({ ctx }) {
  ...
  return <div className='page active'><div style={{ padding: '32px 36px' }}>
    ...
    ) : !masterKeys.length ? <div className='empty'><div className='empty-icon'>{'🔑'}</div><div className='empty-text'>No master keys configured</div><button className='btn btn-primary' disabled={atMasterKeyLimit} onClick={() => setModal('addkey')}>Add your first key</button></div> : masterKeys.map((mk) => <div className='card masterkey-card' key={mk.id}><div className='masterkey-icon' style={{ width: '42px', height: '42px', background: 'var(--bg4)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>{mk.provider === 'openai' ? '⬛' : mk.provider === 'google' ? '🔵' : mk.provider === 'anthropic' ? '🟧' : mk.provider === 'deepseek' ? '🌊' : mk.provider === 'xai' ? '✕' : mk.provider === 'groq' ? '⚡' : '🔐'}</div><div className='masterkey-info' style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: '14px', fontWeight: 600, marginBottom: '3px' }}>{mk.name || providerLabel(providerOptions, mk.provider)}</div><div className='token-val' style={{ fontSize: '12px', color: 'var(--muted)' }}>{mk.key_masked}</div></div><div className='masterkey-meta' style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}><div style={{ fontSize: '11px', color: 'var(--dim)' }}>Added {fmtDate(mk.created_at)}</div><span className='masterkey-provider-badge' style={{ fontSize: '10px', background: 'rgba(34,197,94,0.12)', color: 'var(--green)', padding: '4px 10px', borderRadius: '999px', fontWeight: 600 }}>{providerLabel(providerOptions, mk.provider)}</span><button className='btn btn-icon btn-danger' title='Delete master key' disabled={deletingId === mk.id} onClick={() => deleteMasterKey(mk)} aria-label='Delete master key'>{deletingId === mk.id ? <span className='spinner-small'></span> : <span role='img' aria-label='trash'>🗑️</span>}</button></div></div>)}
