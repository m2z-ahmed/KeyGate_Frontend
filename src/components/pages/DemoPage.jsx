...
export default function DemoPage({ ctx }) {
  ...
  const preview = !selectedSubkey
    ? 'Select a subkey to see the request preview...'
  ...
  const snippetCard = (title, code) => (
    <div className='card copy-btn' key={title} onClick={() => { navigator.clipboard.writeText(code); setCopiedSnippet(title); setTimeout(() => setCopiedSnippet(''), 1600); }} style={{ cursor: 'pointer' }}>
      <div className='card-title' style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {title}
        {copiedSnippet === title ? <><IconCheck width={14} height={14} style={{ color: 'var(--green)' }} /><span style={{ fontSize: 11, color: 'var(--green)', fontWeight: 500 }}>Copied!</span></> : <IconCopy width={14} height={14} style={{ color: 'var(--muted)' }} />}
      </div>
      <pre style={{ whiteSpace: 'pre-wrap', fontFamily: 'DM Mono, monospace', fontSize: '12px', marginTop: '8px', color: 'var(--text)' }}>{code}</pre>
    </div>
  );

  return (
    <div className='page active demo-page'>
      <div className='page-inner'>
        <div className='page-header'>
          <div className='page-title'>Live demo</div>
          <div className='page-sub'>See exactly how a client uses a subkey — without ever knowing the real key</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          <div className='card'>
            <div className='card-header'><div className='card-title'>Configure test call</div></div>
            {!active.length && <div className='empty-text'>No active subkeys.</div>}
            ...
            <div className='field' style={{ marginBottom: 10 }}><label>Provider</label><div className='badge active'>{selectedSubkey ? providerLabel(providerOptions, selectedSubkey.provider) : 'Select a subkey'}</div></div>
            <div className='field' style={{ marginBottom: 10 }}><label>Model</label><select value={model} onChange={(e) => setModel(e.target.value)}>{allowedModelList.map((m) => <option key={m} value={m}>{m}</option>)}</select><div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>Only models allowed by the selected subkey are shown.</div></div>
          ...
          <div className='card' style={{ background: 'var(--bg)' }}>
            <div className='card-title'>What the client sends</div>
            <pre style={{ fontFamily: 'DM Mono, monospace', fontSize: '12px', color: 'var(--text)', lineHeight: 1.9, whiteSpace: 'pre-wrap', marginTop: '8px' }}>{preview}</pre>
          </div>
        </div>
        <div className='console'>
          <div className='console-bar'><div className='dot r' /><div className='dot y' /><div className='dot g' /><span style={{ fontSize: '11px', color: 'var(--muted)', marginLeft: '8px' }}>Lethem proxy console</span></div>
          <div className='console-body'>{consoleLines.map((l, i) => <p key={i} className='console-line'>{l}</p>)}</div>
        </div>
        <div className='snippet-grid'>{snippetCard('JS', jsSnippet)}{snippetCard('Python', pySnippet)}{snippetCard('cURL', curlSnippet)}</div>
      </div>
    </div>
  );
}
