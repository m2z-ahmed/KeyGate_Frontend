          ...
          {ROLES.map((role) => (
            <div className='card role-card' key={role.id}>
              <div className='card-header'>
                <div><div className='card-title'>{role.label}</div><div className='card-sub'>{role.description}</div></div>
                <span className={`badge ${role.tone}`}>{role.id}</span>
              </div>
              <ul className='permission-list'>{role.permissions.map((permission) => <li key={permission}>{permission}</li>)}</ul>
            </div>
          ))}
        </div>
      </div>
