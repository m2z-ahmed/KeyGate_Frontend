...
const pageCopy = {
  ...
  usage: { title: 'Usage', sub: 'Understand plan limits for tokens, projects, subkeys, and retained logs.', status: 'Billing preview', cards: [['Tokens used', '8.2M / 12M', '68% of monthly allowance'], ['Projects used', '1 / 3', 'Free workspace limit'], ['Subkeys used', '14 / 50', 'Across this project']] },
  ...
  endpoint: { title: 'API Endpoint', sub: 'Configure proxy endpoints, environments, and SDK connection details.', status: 'Settings preview', cards: [['Proxy endpoint', API_BASE_URL, 'Production gateway'], ['Environment', 'Production', 'Live traffic enabled'], ['SDK status', 'Ready', 'Use this endpoint in clients']] },
  ...
  danger: { title: 'Danger Zone', sub: 'Destructive actions for deleting projects, revoking access, and leaving organizations.', status: 'Protected preview', cards: [['Delete project', 'Locked', 'Requires confirmation'], ['Revoke all subkeys', 'Locked', 'Emergency access reset'], ['Leave organization', 'Owner blocked', 'Transfer ownership first']] },
  profile: { title: 'Profile', sub: 'View personal identity details supplied by your authentication provider.', status: 'Account preview', cards: [['Name', 'Lethem User', 'Editable in auth provider'], ['Email', 'user@example.com', 'Verified'], ['Auth provider', 'Auth0', 'Single sign-on ready']] },
...
export default function PlaceholderPage({ type = 'general', onBack = null }) {
  ...
  return (
    <div>
      {onBack && <Button variant="ghost" size="sm" onClick={onBack} className="mb-3"><ArrowLeft size={15} /> Back to previous page</Button>}
      <div className="mb-6"><h1 className="font-heading text-2xl font-bold tracking-tight text-gradient">{data.title}</h1><p className="mt-1 text-sm text-muted-foreground">{data.sub}</p></div>

      <Card className="mb-4 p-5"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 text-primary"><Construction size={16} /></div><div><div className="text-sm font-semibold">{data.title} is ready for product wiring</div><div className="text-xs text-muted-foreground mt-0.5">These preview screens reserve the final information architecture so billing, teams, organizations, and settings can be connected without reshuffling navigation later.</div></div></div></Card>

      <div className="grid gap-3 sm:grid-cols-3">
        {data.cards.map(([label, value, helper]) => (
          <Card key={label} className="p-5"><div className="text-xs text-muted-foreground">{label}</div><div className="mt-1 truncate text-lg font-semibold">{value}</div><div className="mt-1 text-xs text-muted-foreground">{helper}</div></Card>
        ))}
      </div>

      {isComingSoon && (
        <Card className="mt-4 border-primary/30 bg-primary/5 p-5">
          <div className="flex items-center gap-2 text-primary"><Construction size={16} /><span className="text-sm font-semibold">Coming Soon</span></div>
          <p className="mt-2 text-xs text-muted-foreground">We'll add this page soon. It will be available in a future update.</p>
        </Card>
      )}
    </div>
  );
