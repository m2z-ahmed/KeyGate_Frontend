...
function writeSession(tokenResponse: any): AuthSession {
...
async function exchangeCodeForToken(code: string, verifier: string): Promise<AuthSession> {
  ...
  if (!response.ok) throw new Error(body.error_description || body.error || `Auth0 token exchange failed (HTTP ${response.status})`);
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async () => {
    if (!isConfigured) {
      setAuthError('Auth0 frontend variables are missing. Set VITE_AUTH0_DOMAIN, VITE_AUTH0_CLIENT_ID, and VITE_AUTH0_AUDIENCE.');
      return;
    }
    ...
    const params = new URLSearchParams({
      response_type: 'code', client_id: auth0ClientId, redirect_uri: redirectUri,
      scope: 'openid profile email', audience: auth0Audience, state,
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    isLoading, authError, isConfigured, login, logout, getAccessToken,
