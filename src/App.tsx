...
function AppError({ error, onRetry }: { error: any; onRetry: () => void }) {
  return (
...
function BootLoader({ go, view, projectSlug, onBootComplete }: any) {
  const { loadProviders, loadProjects, loadBilling, loadAccount, notify } = useLethem() as any;
  const [bootFailed, setBootFailed] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([loadProjects(), loadBilling?.().catch(() => null), loadAccount?.().catch(() => null)])
      .then(([list, billing]: any) => {
        ...
        const currentPlan = billing?.plans?.find((plan: any) => plan.id === billing.currentPlan);
        ...
        if (view === 'account') { onBootComplete?.(); return; }
        if (projectSlug && !list.find((p: any) => p.slug === projectSlug || p.id === projectSlug)) {
          notify('Project not found', 'error'); go('/console');
        }
        onBootComplete?.();
      })
      .catch((e: any) => { if (cancelled) return; setBootFailed(e); onBootComplete?.(); });
...
function ViewTransition({ view, children }: any) {
  return <div key={view} className='view-transition'>{children}</div>;
}

function AppRouter({ routeState }: any) {
  const { page, view, projectSlug, go, isPublicHealth, publicPage } = routeState;
  const { ctx } = useLethem() as any;
  ...
  if (isPublicHealth) {
    const publicCtx = { ...ctx, api: (path: any, opts: any = {}) => ctx.api(path, { ...opts, skipAuth: true, headers: {} }) };
...
function AppInner() {
  const routeState = useConsoleRouteState() as any;
  ...
  if (hasError) return <AppError error={hasError} onRetry={() => setHasError(null)} />;
    ...
    <LethemProvider projectSlug={isPublicHealth ? '' : projectSlug} page={page}>
      <div className='app-shell'>
        {publicPage ? (
          ...
          <>
            {!booted && (
              <BootLoader go={routeState.go} view={routeState.view} projectSlug={projectSlug} onBootComplete={() => setBooted(true)} />
            )}
